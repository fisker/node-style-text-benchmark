...
// Copied from https://github.com/sindresorhus/yoctocolors/blob/main/benchmark.js
...
suite
	.addTest('node:util.styleText', () => {
		out = util.styleText('red', 'Add plugin to use time limit');
		out = util.styleText('green', 'Add plugin to use time limit');
		out = util.styleText('blue', `Add plugin to ${util.styleText('cyan', 'use')} time limit`);
	})
	.addTest('node-style-text', () => {
		out = styleText.red('Add plugin to use time limit');
		out = styleText.green('Add plugin to use time limit');
		out = styleText.blue(`Add plugin to ${styleText.cyan('use')} time limit`);
	})
	.addTest('yoctocolors', () => {
